        ...
        [(fun? e)
         (let ((name (fun-name e))
           ...
           (cond [(triggered? body) body]
                 ...
                 [#t (closure (filter-pairs-by-variables env (collect-variable-names e)) (fun name farg body))]))]
             ...
             [(closure? func)
              (let* ([f (closure-f func)]
                ...
                (cond [(not (= (length fargs) (length args))) (triggered (exception "call: arity mismatch"))]
                      ...
                      [#t (fri (vars (append fargs (list name)) (append args (list f)) body) env)]))]
...
(define (missing-variable env var body)
  (let ((vars (append (get-first-elements env) var)))
    (cond [(trigger? body) #f]
          [(exception? body) #f]
          [(valof? body) (not (member (valof-s body) vars))]
          [(vars? body)
           (let ([s (vars-s body)]
                 [e1 (vars-e1 body)]
                 [e2 (vars-e2 body)])
...
(define (collect-variable-names expr)
  (define (helper expr)
    (cond
      [(vars? expr) (append (list (vars-s expr)) (helper (vars-e1 expr)) (helper (vars-e2 expr)))]
      [(fun? expr) (append (fun-farg expr) (helper (fun-body expr)))]
      [(add? expr) (append (helper (add-e1 expr)) (helper (add-e2 expr)))]
      [(mul? expr) (append (helper (mul-e1 expr)) (helper (mul-e2 expr)))]
      [(?leq? expr) (append (helper (?leq-e1 expr)) (helper (?leq-e2 expr)))]
      [(?=? expr) (append (helper (?=-e1 expr)) (helper (?=-e2 expr)))]
      [(..? expr) (append (helper (..-e1 expr)) (helper (..-e2 expr)))]
      [(if-then-else? expr) (append (helper (if-then-else-e1 expr)) (helper (if-then-else-e2 expr)))]
      [(head? expr) (helper (head-e expr))]
      [(tail? expr) (helper (tail-e expr))]
      [(~? expr) (helper (~-e expr))]
      [(?all? expr) (helper (?all-e expr))]
      [(?any? expr) (helper (?any-e expr))]
      [else '()]))
  (apply append (map helper (list expr))))

(define (filter-pairs-by-variables pairs variables)
  (filter (lambda (pair)
            (not (member (car pair) variables)))
          pairs))
    ...
    (if (triggered? v)
        ...
        (letrec ((mapping-aux
                  (lambda (f seq)
                    (if (empty? seq)
                        (empty)
                        (add (.. (call f (list (..-e1 seq))) (empty))
                             (mapping-aux f (..-e2 seq)))))))
          (mapping-aux f v)))))
        ...
        (letrec ((filtering-aux
                  (lambda (f seq)
                    (cond [(empty? seq) (empty)]
                          [(equal? (fri (call f (list (..-e1 seq))) null) (true))
                           (add (.. (..-e1 seq) (empty))
                                (filtering-aux f (..-e2 seq)))]
                          [#t (filtering-aux f (..-e2 seq))]))))
          (filtering-aux f v)))))
    ...
    (if (triggered? v)
        ...
        (letrec ((folding-aux
                  (lambda (f init seq)
                   (if (empty? seq)
                        init
                        (folding-aux f (call f (list (..-e1 seq) init)) (..-e2 seq))))))

...
#	Use these lines for download with progress indication
	...
	wget -c --progress=dot -P C:/Lucija/isrm/Obdelava_biomedicinskih_podatkov_in_slik/Signals/LTSTDB/ $SERVER$r".hea" 2>&1 | grep --line-buffered "%" | sed -u -e "s,\.,,g" | awk '{printf("\b\b\b\b%4s", $2)}'
	echo "Downloading .atr file for for record $r ..."
	wget -c --progress=dot -P C:/Lucija/isrm/Obdelava_biomedicinskih_podatkov_in_slik/Signals/LTSTDB/ $SERVER$r".atr" 2>&1 | grep --line-buffered "%" | sed -u -e "s,\.,,g" | awk '{printf("\b\b\b\b%4s", $2)}'
	echo "Downloading .dat file for for record $r ..."
	wget -c --progress=dot -P C:/Lucija/isrm/Obdelava_biomedicinskih_podatkov_in_slik/Signals/LTSTDB/ $SERVER$r".dat" 2>&1 | grep --line-buffered "%" | sed -u -e "s,\.,,g" | awk '{printf("\b\b\b\b%4s", $2)}'

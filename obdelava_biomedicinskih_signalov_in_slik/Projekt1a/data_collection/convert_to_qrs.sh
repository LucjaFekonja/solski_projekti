...
#	wget -c --progress=dot -P c:\Lucija\isrm\Obdelava_biomedicinskih_podatkov_in_slik\Signals\MIT-BIT $SERVER$r".hea" 2>&1 | grep --line-buffered "%" | sed -u -e "s,\.,,g" | awk '{printf("\b\b\b\b%4s", $2)}'
...
#	wget -q --progress=bar:force -c $SERVER$r".dat"
	...
	echo "Converting .asc to WFDB format ..."
	wrann -r $r -a qrs < $r.asc
	echo "--------------------------------------------------"
done
